...
setTimeout(async () => {
    console.log('Bắt đầu check PMT và sử dụng');
    ...
    for (let i = 0; i < COOKIES.length; i++) {
        ...
        const event = await chuyenDo(CM_COOKIE, userid, PMT, 1);
        if (event == "Bảo khố không có vật phẩm này hoặc đã sử dụng hết") {
            PMT -= 1;
            await chuyenDo(CM_COOKIE, userid, PMT, 1);
        }
        await delay(3000)
        await fetchRuong(cookie);
        await delay(5000)
    }
    console.log('End!!!');
}, 1000);
...
const fetchRuong = async (cookie) => {
    ...
    let dsVatpham = [
        "Phụ Ma Thạch <sup>C8</sup>",
        "Phụ Ma Thạch <sup>C7</sup>",
        "Phụ Ma Thạch <sup>C6</sup>",
        "Phụ Ma Thạch <sup>C5</sup>",
        "Phụ Ma Thạch <sup>C4</sup>",
        "Phụ Ma Thạch <sup>C3</sup>",
        "Phụ Ma Thạch <sup>C2</sup>",
        "Phụ Ma Thạch <sup>C1</sup>",
    ];
    let idVatpham = [
        15791,
        15790,
    ...
    //const ltd = $('span#shopnum77').text();
    //await hanhTrang(cookie, ltd);
    //await delay(1000)
    for (let i = 0; i < dsVatpham.length; i++) {
        if (body.includes(dsVatpham[i])) {
            console.log('Bạn đang có ' + dsVatpham[i])
            await suDung(cookie, idVatpham[i])
            break;
        }
    }
}

const suDung = async (cookie, itemId) => {
    const body = 'btnTangExpBac=1&items=' + itemId + '&txtSoLuong=1';
    const response = await fetch("https://tutien.net/account/vat_pham/", {
        "headers": {
            "accept": "*/*",
            "accept-language": "en-US,en;q=0.9,vi;q=0.8",
    ...
    if (res.includes('data')) {
        ...
        console.log("Đã sử dụng hoặc có lỗi xảy ra")
    }
}
const hanhTrang = async (cookie, txtNumber) => {
    const body = 'btnHanhTrang=1&shop=77&txtNumber2=' + txtNumber;
    const response = await fetch("https://tutien.net/account/vat_pham/", {
        "headers": {
            "accept": "*/*",
            "accept-language": "en-US,en;q=0.9,vi;q=0.8",
    ...
    const res = await response.text();
    if (res == 1) {
        console.log("Đã đưa vào hành trang!")
    } else
